...
class SaveFlights(forms.ModelForm):
    ...
    def clean_airline(self):
        ...
        try:
            ...
            raise forms.ValidationError(f"The selected airline is invalid")
    ...
    def clean_from_airport(self):
        ...
        try:
            ...
            raise forms.ValidationError(f"The selected From Airport is invalid")
...
class SaveReservation(forms.ModelForm):
    ...
    luggage_type = forms.ChoiceField(
        choices=models.Reservation.LUGGAGE_CHOICES,
        initial='hand_carry'
    )
    luggage_weight = forms.DecimalField(
        max_digits=6,
        decimal_places=2,
        min_value=0,
        max_value=32,
    ...
    class Meta:
        ...
        fields = ('flight', 'type', 'first_name', 'middle_name', 'last_name', 'gender', 
                 'contact', 'email', 'address', 'luggage_type', 'luggage_weight')
    ...
    def clean_flight(self):
        ...
        try:
            ...
            raise forms.ValidationError("Invalid Flight")

    def clean_luggage_weight(self):
        luggage_type = self.cleaned_data.get('luggage_type')
        luggage_weight = self.cleaned_data.get('luggage_weight')
        
        if luggage_type and luggage_weight:
            if luggage_type == 'hand_carry' and luggage_weight > 7:
                raise forms.ValidationError("Hand carry cannot exceed 7kg")
            elif luggage_type == 'checked_bag' and luggage_weight > 23:
                raise forms.ValidationError("Checked baggage cannot exceed 23kg")
            elif luggage_type == 'extra_bag' and luggage_weight > 32:
                raise forms.ValidationError("Extra baggage cannot exceed 32kg")
